        ...
        switch (event.type) {
            case SDL_JOYDEVICEADDED:
                if (!SDL_IsGameController(event.jdevice.which)) {
                    std::cout << "Gamepad isn't supported. "
                              << "Add a mapping into gamecontrollerdb.txt file."
                              << std::endl;
                    core->log("Устройство не поддерживается. Добавьте " \
                              "mapping для данного устройства в файл " \
                              "gamecontrollerdb.txt в корневую папку драйвера");
                    break;
                }
                OnJoystickConnected(event.jdevice.which);
                break;
